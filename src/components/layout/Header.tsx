...
export const Header = () => {
  ...
  const isDropdownActive = (label: string, pathname: string) => {
    if (label === "Dịch vụ & Sản phẩm") {
      return ["/dich-vu-cot-loi", "/dich-vu-khac", "/san-pham"].some(path => pathname.startsWith(path));
    }
    if (label === "Chính sách") {
    ...
    <div className="relative w-full flex justify-center z-50 bg-white shadow-sm font-inter">
      ...
      <header className="lg:hidden relative w-full h-[72px] md:h-[80px] bg-white border-b border-[#FF0000] flex items-center justify-between px-[20px] md:px-[32px]">
        <div className="flex-shrink-0 flex items-center gap-2">
          <img src={qr} alt="QR" className="w-[30px] h-[30px] object-contain" />
          <NavLink to="/" onClick={closeMenu}>
            <img src={logo} alt="Promac" className="w-[100px] h-auto md:w-[120px] object-contain" />
          </NavLink>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="fixed top-[72px] md:top-[80px] left-0 w-full h-[calc(100vh-72px)] md:h-[calc(100vh-80px)] bg-white z-40 lg:hidden overflow-hidden border-t border-[#E3E7EF]">
          {/* VIEW 1: MAIN MENU */}
          <div className={cn("absolute inset-0 w-full h-full bg-white transition-transform duration-300 ease-in-out flex flex-col overflow-y-auto pb-[40px]", currentView === "main" ? "translate-x-0" : "-translate-x-full")}>
            {NAV_ITEMS.map((item, index) => {
              if (item.type === "dropdown") {
                return (
                  <button
                    ...
                    className="flex w-full items-center justify-between bg-white transition-colors active:bg-gray-50 border-b border-[#E3E7EF] px-[24px] md:px-[40px] py-[16px]"
                  >
                    <span className="font-semibold text-[16px] md:text-[18px] text-[#111B29]">{item.label}</span>
                    <ChevronRight size={20} className="text-[#9CA3AF]" />
                  </button>
                );
              }
              return (
                <NavLink
                  ...
                  to={item.path}
                  onClick={closeMenu}
                  className={({ isActive }) => cn("flex w-full items-center justify-between bg-white transition-colors active:bg-gray-50 border-b border-[#E3E7EF] px-[24px] md:px-[40px] py-[16px]", isActive ? "text-[#FF0000] bg-red-50/50" : "text-[#111B29]")}
                >
                    <div className="flex items-center gap-2">
                        {item.icon && <span className="text-current">{item.icon}</span>}
                        <span className="font-semibold text-[16px] md:text-[18px]">{item.label}</span>
                    </div>
                </NavLink>
              );
            })}
             ...
             <div className="mt-[10px]">
                <NavLink to="/bang-gia" onClick={closeMenu} className={({ isActive }) => cn("flex w-full items-center justify-between bg-white transition-colors active:bg-gray-50 border-b border-[#E3E7EF] px-[24px] md:px-[40px] py-[16px]", isActive ? "text-[#FF0000] bg-red-50/50" : "text-[#111B29]")}>
                    <div className="flex items-center gap-2">
                        <CalendarDays size={20} />
                        <span className="font-semibold text-[16px] md:text-[18px]">Bảng giá</span>
                    </div>
                </NavLink>
             </div>
          </div>
          ...
          {/* VIEW 2: SUB MENU */}
          {Object.keys(MOBILE_SUB_MENUS).map((menuLabel) => (
            <div key={menuLabel} className={cn("absolute inset-0 w-full h-full bg-white transition-transform duration-300 ease-in-out flex flex-col overflow-y-auto pb-[40px]", currentView === menuLabel ? "translate-x-0" : "translate-x-full")}>
              <div className="flex items-center gap-[10px] w-full bg-gray-50 border-b border-[#E3E7EF] px-[24px] md:px-[40px] py-[16px] sticky top-0 z-10">
                ...
                <span className="font-bold text-[18px] md:text-[20px] text-[#111B29]">{menuLabel}</span>
              </div>
              ...
              <div className="flex flex-col w-full py-[10px]">
                {MOBILE_SUB_MENUS[menuLabel].map((group, gIdx) => (
                  <div key={gIdx} className="flex flex-col mb-[20px]">
                    {group.groupTitle && (
                      <div className="px-[24px] md:px-[40px] mb-[8px] mt-[10px] font-bold text-[12px] md:text-[14px] text-[#6B7280] uppercase tracking-wider">{group.groupTitle}</div>
                    )}
                    {group.items.map((sub, sIdx) => (
                      <NavLink key={sIdx} to={sub.path} onClick={closeMenu} className={({ isActive }) => cn("flex items-center py-[12px] px-[24px] md:px-[40px] text-[16px] md:text-[18px] font-medium transition-colors hover:bg-gray-50", isActive ? "text-[#FF0000] bg-red-50/30" : "text-[#374151]")}>
                        {sub.label}
                      </NavLink>
                    ))}
                    {group.bottomImage && (
                      <div className="mt-[20px] ml-[24px] md:ml-[40px]">
                        <img src={group.bottomImage} alt="QR Code" className="w-[120px] h-[120px] object-contain border border-gray-200 p-2 rounded-lg" />
                      </div>
                    )}
                  </div>
                ))}
